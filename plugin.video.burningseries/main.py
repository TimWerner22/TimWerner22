	...
	while True:
		...
		if data.has_key('series'):
			if seasonName == "[B] Staffel 0[/B]":
				addDirectoryItem("[B] Specials[/B]", {"kindOf":2, "name":data['series']['series'].encode('utf-8'), "id":id, "season":season, "desc":data['series']['description'].encode('utf-8')},cover)
			else:
				addDirectoryItem(seasonName, {"kindOf":2, "name":data['series']['series'].encode('utf-8'), "id":id, "season":season, "desc":data['series']['description'].encode('utf-8')},cover)
			season+=1
...
def showEpisodes(n,id,season):
	...
	for d in data['epi']:
		...
		deschelp = json.loads(getUrl("series/"+str(id)+"/"+str(season)+"/"+str(d['epi'])))
		addDirectoryItem(episodeName_watched, {"kindOf": 3, "name":data['series']['series'].encode('utf-8'), "id":id, "season":season, "episode":d['epi'],"episodename":episodeName.encode('utf-8'), "desc":deschelp['epi']['description'].encode('utf-8')},cover)
...
def showHosts(n, id, season,episode,episodeName):
	...
	print data
	hoster_list = hosterList.split(', ')
	for d in data['links']:
		if d['hoster'].lower() in hoster_list:
...
def addDirectoryItem(name, parameters={},pic=""):
	...
	if parameters.has_key('desc'):
		li.setInfo(type="Video", infoLabels={"Plot":parameters['desc']})
